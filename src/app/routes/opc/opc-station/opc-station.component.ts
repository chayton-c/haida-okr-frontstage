...
@Component({
  selector: 'app-opc-opc-station',
  templateUrl: './opc-station.component.html',
})
export class OpcOpcStationComponent implements OnInit {

  ngOnInit(): void {

    this.activatedRoute.queryParams.subscribe((queryParams) => {
      if (queryParams.checkedStationId) this.checkedStationId = queryParams.checkedStationId;
    });
  ...
  clickLineNodePojoById(id: string) {
    let item: LineNodePojo|undefined;
  ...
  loadRailwayLineDataFromServer(): void {
    this.http.post('/api/backstage/railwayLine', null).subscribe((res) => {
      ...
      if (this.checkedStationId) this.clickLineNodePojoById(this.checkedStationId);
    });
  }
